
const BASE_URL = "https://api.vaiacharms.gr/wp-json/wc/v3";
const CK = "ck_a60909f612e329245b86ce88e876b4928cf8d419";
const CS = "cs_8509165bda4837aed2a6f1c0d03d0e7d35570809";

async function getCategories() {
    const url = `${BASE_URL}/products/categories?per_page=100&consumer_key=${CK}&consumer_secret=${CS}`;
    const res = await fetch(url);
    return await res.json();
}

async function getTags() {
    const url = `${BASE_URL}/products/tags?per_page=100&consumer_key=${CK}&consumer_secret=${CS}`;
    const res = await fetch(url);
    return await res.json();
}

Promise.all([getCategories(), getTags()]).then(([cats, tags]) => {
    console.log("=== CATEGORIES ===");
    cats.forEach(c => console.log(`ID: ${c.id} | Name: ${c.name} | Count: ${c.count}`));
    console.log("\n=== TAGS ===");
    tags.forEach(t => console.log(`ID: ${t.id} | Name: ${t.name} | Count: ${t.count}`));
}).catch(err => console.error(err));
