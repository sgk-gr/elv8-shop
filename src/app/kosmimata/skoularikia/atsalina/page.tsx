...
export default function SkoularikiaAtsalinaPage() {
    // Φέρνουμε τα προϊόντα για την κατηγορία Ατσάλινα (ID: 96) κάτω από τα Σκουλαρίκια
    const { data: products = [], isLoading } = useQuery<WooProduct[]>({
        queryKey: ["products", { category: "96" }],
        queryFn: () => getProducts({ per_page: "100", category: "96" }),
    });
    ...
    return (
        <main className="container mx-auto px-4 md:px-8 py-12 md:py-20">
            ...
            <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-8">
                ...
                <ChevronRight className="w-4 h-4" />
                <Link href="/kosmimata/skoularikia" className="hover:text-primary transition-colors">Σκουλαρίκια</Link>
                <ChevronRight className="w-4 h-4" />
                <span className="text-primary font-medium">Ατσάλινα</span>
            </nav>

            <div className="space-y-4 mb-12">
                <h1 className="font-display text-4xl md:text-5xl font-bold tracking-tight text-[#C4196D]">
                    Σκουλαρίκια Ατσάλινα
                </h1>
                <p className="text-muted-foreground font-body max-w-2xl leading-relaxed">
                    Ανακαλύψτε τη συλλογή μας από εντυπωσιακά ατσάλινα σκουλαρίκια (Stainless Steel).
                    Υποαλλεργικά και ανθεκτικά, είναι η ιδανική επιλογή για να προσθέσετε λάμψη και στυλ στην κάθε σας μέρα.
                </p>
            ...
            {products.length === 0 && !isLoading ? (
                (<div className="text-center py-32 space-y-4 bg-secondary/20 rounded-[2rem] border border-dashed border-border">
                    ...
                    <Link href="/kosmimata/skoularikia" className="inline-block text-primary font-body text-sm font-semibold border-b border-primary">
                        Δείτε όλα τα σκουλαρίκια
                    </Link>
                </div>)
            ) : (
