...
export default function KolieStainlessSteelPage() {
    // Φέρνουμε τα προϊόντα για την κατηγορία Stainless Steel (ID: 71)
    const { data: products = [], isLoading } = useQuery<WooProduct[]>({
        queryKey: ["products", { category: "71" }],
        queryFn: () => getProducts({ per_page: "100", category: "71" }),
    });
    ...
    return (
        <main className="container mx-auto px-4 md:px-8 py-12 md:py-20">
            ...
            <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-8">
                ...
                <ChevronRight className="w-4 h-4" />
                <Link href="/products?category=18" className="hover:text-primary transition-colors">Κολιέ</Link>
                <ChevronRight className="w-4 h-4" />
            ...
            <div className="space-y-4 mb-12">
                <h1 className="font-display text-4xl md:text-5xl font-bold tracking-tight text-[#C4196D]">
                    Κολιέ Stainless Steel
                </h1>
                <p className="text-muted-foreground font-body max-w-2xl leading-relaxed">
                    Ανακαλύψτε τη συλλογή μας από εντυπωσιακά κολιέ από ανοξείδωτο ατσάλι (Stainless Steel). 
            ...
            {products.length === 0 && !isLoading ? (
                (<div className="text-center py-32 space-y-4 bg-secondary/20 rounded-[2rem] border border-dashed border-border">
                    ...
                    <Link href="/products?category=18" className="inline-block text-primary font-body text-sm font-semibold border-b border-primary">
                        Δείτε όλα τα κολιέ
                    </Link>
                </div>)
            ) : (
