...
export default function DaxtilidiaStainlessSteelPage() {
    // Φέρνουμε τα προϊόντα για την κατηγορία Stainless Steel (ID: 74)
    const { data: products = [], isLoading } = useQuery<WooProduct[]>({
        queryKey: ["products", { category: "74" }],
        queryFn: () => getProducts({ per_page: "100", category: "74" }),
    });
    ...
    return (
        <main className="container mx-auto px-4 md:px-8 py-12 md:py-20">
            ...
            <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-8">
                ...
                <ChevronRight className="w-4 h-4" />
                <Link href="/kosmimata/daxtilidia" className="hover:text-primary transition-colors">Δαχτυλίδια</Link>
                <ChevronRight className="w-4 h-4" />
                <span className="text-primary font-medium">Stainless Steel</span>
            </nav>

            <div className="space-y-4 mb-12">
                <h1 className="font-display text-4xl md:text-5xl font-bold tracking-tight text-[#C4196D]">
                    Δαχτυλίδια Stainless Steel
                </h1>
                <p className="text-muted-foreground font-body max-w-2xl leading-relaxed">
                    Ανακαλύψτε τη συλλογή μας από εντυπωσιακά δαχτυλίδια από ανοξείδωτο ατσάλι (Stainless Steel). 
                    Σχεδιασμένα για να αντέχουν στο χρόνο χωρίς να χάνουν τη λάμψη τους, είναι η τέλεια επιλογή για καθημερινή κομψότητα.
                </p>
            ...
            {isLoading ? (
                ...
                <div className="text-center py-32 space-y-4 bg-secondary/20 rounded-[2rem] border border-dashed border-border">
                    ...
                    <Link href="/kosmimata/daxtilidia" className="inline-block text-primary font-body text-sm font-semibold border-b border-primary">
                        Δείτε όλα τα δαχτυλίδια
                    </Link>
                </div>
            ) : (
