...
export default function LipsticksPage() {
    // Φέρνουμε τα προϊόντα για την κατηγορία Lipsticks (ID: 21)
    const { data: products = [], isLoading } = useQuery<WooProduct[]>({
        queryKey: ["products", { category: "21" }],
        queryFn: () => getProducts({ per_page: "100", category: "21" }),
    });
    ...
    return (
        <main className="container mx-auto px-4 md:px-8 py-12 md:py-20">
            ...
            <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-8">
                ...
                <Link href="/products?category=20" className="hover:text-primary transition-colors">Cosmetics</Link>
                <ChevronRight className="w-4 h-4" />
                <span className="text-primary font-medium">Lipsticks</span>
            </nav>
            ...
            <div className="space-y-4 mb-12">
                ...
                <p className="text-muted-foreground font-body max-w-2xl leading-relaxed">
                    Ανακαλύψτε τη συλλογή μας από καλλυντικά και βρείτε το ιδανικό lipstick 
                    που θα αναδείξει την ομορφιά σας με ακαταμάχητο χρώμα.
                </p>
            ...
            {isLoading ? (
                ...
                <div className="text-center py-32 space-y-4 bg-secondary/20 rounded-[2rem] border border-dashed border-border">
                    ...
                    <Link href="/products?category=20" className="inline-block text-primary font-body text-sm font-semibold border-b border-primary">
                        Δείτε όλα τα Cosmetics
                    </Link>
                </div>
            ) : (
