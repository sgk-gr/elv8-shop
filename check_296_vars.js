const BASE_URL = "https://api.vaiacharms.gr/wp-json/wc/v3";
const CK = "ck_a60909f612e329245b86ce88e876b4928cf8d419";
const CS = "cs_8509165bda4837aed2a6f1c0d03d0e7d35570809";

async function getVariations(id) {
    const url = `${BASE_URL}/products/${id}/variations?consumer_key=${CK}&consumer_secret=${CS}`;
    const res = await fetch(url);
    return await res.json();
}

getVariations(296).then(vars => {
    vars.forEach(v => {
        console.log(`ID: ${v.id}, SKU: ${v.sku}, Price: ${v.price}`);
        console.log(`Attributes: ${JSON.stringify(v.attributes)}`);
        console.log(`Image: ${v.image ? v.image.src : 'No image'}`);
        console.log('---');
    });
});
